...
def generate_launch_description():
    ...
    # 地图属性以及是否使用动力学仿真
    map_type = LaunchConfiguration('map_type', default="map_generator") # map_generator or mockamap 
    map_type_arg = DeclareLaunchArgument('map_type', default_value=map_type, description='Choose map type, map_generator or mockamap')
    ...
    random_forest_node = Node(
        ...
        condition = IfCondition(EqualsSubstitution(map_type, 'map_generator'))
    )
    ...
    mockamap_node = Node(
        ...
        condition = IfCondition(EqualsSubstitution(map_type, 'mockamap'))
    )
    ...
    so3_control_component = launch_ros.descriptions.ComposableNode(
            ...
            condition = IfCondition(EqualsSubstitution(use_dynamic, "True"))
            )
    ...
    poscmd_2_odom_node = Node(
        ...
        condition = IfCondition(EqualsSubstitution(use_dynamic, "False"))
    )
    ...
    camera_file = os.path.join( get_package_share_directory('local_sensing'), 'params', 'camera.yaml' )
